...
namespace Viettel
{
    public abstract partial class DbContext : IDbContext, IDisposable
    {
        ...
        public virtual TEntity Insert<TEntity>(TEntity entity, string table)
        {
            ...
            {
                /* 主键为空并且主键又不是自增列 */
                throw new ViettelException(string.Format("The primary key '{0}' could not be null.", nullValueKey.Property.Name));
            }
            ...
            {
                throw new ViettelException("Unable to get the identity value.");
            }
        ...
        public virtual object Insert<TEntity>(Expression<Func<TEntity>> content, string table)
        {
            ...
            foreach (var kv in insertColumns)
            {
                ...
                if (propertyDescriptor == null)
                    throw new ViettelException(string.Format("The member '{0}' does not map any column.", key.Name));

                if (propertyDescriptor.IsAutoIncrement)
                    throw new ViettelException(string.Format("Could not insert value into the identity column '{0}'.", propertyDescriptor.Column.Name));

                if (propertyDescriptor.IsPrimaryKey)
                {
                    object val = ExpressionEvaluator.Evaluate(kv.Value);
                    if (val == null)
                        throw new ViettelException(string.Format("The primary key '{0}' could not be null.", propertyDescriptor.Property.Name));
                    else
                    {
                        keyVal = val;
            ...
            if (keyPropertyDescriptor != null)
            {
                ...
                {
                    throw new ViettelException(string.Format("The primary key '{0}' could not be null.", keyPropertyDescriptor.Property.Name));
                }
            }
        ...
        public virtual int Update<TEntity>(Expression<Func<TEntity, bool>> condition, Expression<Func<TEntity, TEntity>> content, string table)
        {
            ...
            foreach (var kv in updateColumns)
            {
                ...
                if (propertyDescriptor.IsPrimaryKey)
                    throw new ViettelException(string.Format("Could not update the primary key '{0}'.", propertyDescriptor.Column.Name));

                if (propertyDescriptor.IsAutoIncrement)
                    throw new ViettelException(string.Format("Could not update the identity column '{0}'.", propertyDescriptor.Column.Name));
