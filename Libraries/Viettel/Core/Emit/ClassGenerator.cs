        ...
        public static Type CreateMRMType(MemberInfo propertyOrField)
        {
            ...
            if (!_moduleBuilders.TryGetValue(assembly, out moduleBuilder))
            {
                lock (assembly)
                {
                    if (!_moduleBuilders.TryGetValue(assembly, out moduleBuilder))
                    {
                        var assemblyName = new AssemblyName(String.Format(CultureInfo.InvariantCulture, "ViettelMRMs-{0}", assembly.FullName));
...
#endif
                        moduleBuilder = assemblyBuilder.DefineDynamicModule("ViettelMRMModule");
            ...
            TypeBuilder tb = moduleBuilder.DefineType(string.Format("Viettel.Mapper.MRMs.{0}_{1}_{2}", entityType.Name, propertyOrField.Name, Guid.NewGuid().ToString("N").Substring(0, 5) + System.Threading.Interlocked.Increment(ref _sequenceNumber).ToString()), typeAttributes, null, new Type[] { typeof(IMRM) });
