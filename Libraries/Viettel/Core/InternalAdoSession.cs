...
namespace Viettel.Core
{
    class InternalAdoSession : IDisposable
    {
        ...
        public void CommitTransaction()
        {
            if (!this._isInTransaction)
            {
                throw new ViettelException("Current session does not open a transaction.");
            }
        ...
        static ViettelException WrapException(Exception ex)
        {
            return new ViettelException("An exception occurred while executing DbCommand. For details please see the inner exception.", ex);
        }
    }
}
