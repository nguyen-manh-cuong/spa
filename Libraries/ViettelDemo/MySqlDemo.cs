...
namespace ViettelDemo
{
    class MySqlDemo
    {
        ...
        static DbContext context = new MySqlContext(new MySqlConnectionFactory("Database='Viettel';Data Source=localhost;User ID=root;Password=sasa;CharSet=utf8;SslMode=None"));
        ...
        public static void ExecuteCommandText()
        {
            ...
            int rowsAffected = context.Session.ExecuteNonQuery("update Users set name=?name where Id = 1", DbParam.Create("?name", "Viettel"));

            /* 
             * 执行存储过程:
             * User user = context.SqlQuery<User>("Proc_GetUser", CommandType.StoredProcedure, DbParam.Create("?id", 1)).FirstOrDefault();
             * rowsAffected = context.Session.ExecuteNonQuery("Proc_UpdateUserName", CommandType.StoredProcedure, DbParam.Create("?name", "Viettel"));
             */
