...
namespace AuthTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var resWithoutToken = client.GetAsync("/users").Result;

            Console.WriteLine($"Sending Request to /users , without token.");
            ...
            var resWithToken = client.GetAsync("/users").Result;

            Console.WriteLine($"\nSend Request to /users , with token.");
            ...
            client.DefaultRequestHeaders.Clear();
            var res = client.GetAsync("/users/1").Result;

            Console.WriteLine($"Send Request to /users/1");
