  ...
  <mat-form-field appearance="{{appearance}}" class="{{class}}">
    ...
    <div class="box-search">
      ...
      <mat-select-trigger>
        ...
        <mat-option *ngFor="let option of options" [attr.disabled]="option.disabled" [value]="option[value]"
...
export class SelectAutocompleteComponent implements OnChanges, DoCheck {
    ...
    if (this.selectedName && this.selectedName.length) {
      ...
      if (this.multiple) {
        ...
        for (let i = 0; i < this.labelCount; i++) {
          displayOption[i] = this.selectedName[i];
        }
        ...
        displayOption =  this.selectedName;
        if (displayOption.length) {
  ...
  onSelectionChange(val) {
    ...
    setTimeout(() => {
      this.selectedName = this.optionSelected;
    }, 100);
    this.selectionChange.emit(this.selectedValue);
  }
