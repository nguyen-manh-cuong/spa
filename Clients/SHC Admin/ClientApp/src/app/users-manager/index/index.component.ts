...
export class IndexComponent extends PagedListingComponentBase<IUser> implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.dataService.get('groups-all', JSON.stringify({ 'appId': '2'}), '', null, null).subscribe(resp => this.groups = resp.items);
    }
    ...
    onHandleApproved(user: any, event) {
        ...
        if (user.status === 0) {
            return swal({
                ...
                if (result.value) {
                    this.dataService.get(this.api, JSON.stringify(this.frmSearch.value), JSON.stringify({ 'userId': 'desc' }), this.paginator.pageIndex, this.paginator.pageSize)
