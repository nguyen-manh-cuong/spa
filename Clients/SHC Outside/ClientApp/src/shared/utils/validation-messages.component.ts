...
export class ValidationMessagesComponent implements AfterViewInit {
    ...
    ngAfterViewInit(): void {
        console.log(this.formCtrl, this._elementRef.nativeElement,  $(this.formCtrl).attr('name'));
        setTimeout(() => {
            let targetElements = $(this._elementRef.nativeElement).parent().find('[name=\'' + $(this.formCtrl).attr('name') + '\']');
            ...
            if (targetElement.attr('required')) {
                this.errorDefs.push({ required: 'Không được để trống' });
