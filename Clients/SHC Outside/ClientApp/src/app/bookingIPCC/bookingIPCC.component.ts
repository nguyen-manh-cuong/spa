...
export class BookingIPCCComponent extends AppComponentBase implements OnInit {
  ...
  onblur($event) {
    ...
    if ($event.target.value.length >= 2 && $event.target.value.length <= 3) {
      hour = $event.target.value.substring(0, 2);
      $event.target.value = hour + ":00";
    }
    if ($event.target.value.length == 4) {
      hour = $event.target.value.substring(0, 2);
      minute = $event.target.value.substring($event.target.value.indexOf(':') + 1, 4);
      $event.target.value = hour + ":" + (parseInt(minute) >= 10 ? minute : "0" + minute);
    }
  }
