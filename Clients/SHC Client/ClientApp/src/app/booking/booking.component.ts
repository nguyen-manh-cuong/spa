...
export class BookingComponent extends AppComponentBase implements OnInit, AfterViewInit {
        ...
        setTimeout(() => {
            this._dataService
                ...
                .subscribe(resp => {
                    ...
                    if (this._provinces.filter(p => p.provinceCode == provinceCode[0].values).length === 0) {
                        provinceCode[0].values = this._provinces[0].provinceCode;
                    }

                    this.onSelectProvince(provinceCode[0].values);
                    this.frmBooking.controls['provinceCode'].setValue(provinceCode[0].values);

                    this._dataService.get('districts', JSON.stringify({ ProvinceCode: provinceCode[0].values }), '', 0, 0).subscribe(resp => {
                        if (resp.items.filter(d => d.districtCode == districtCode[0].values).length === 0) {
                            this.frmBooking.controls['districtCode'].setValue(resp.items[0].districtCode);
                        }
                        else {
    ...
    onSelectProvince(obj: any) {
        ...
        if (province) {
            this._dataService.get('districts', JSON.stringify({ ProvinceCode: province.provinceCode }), '', 0, 0).subscribe(resp => this._districts = resp.items);
        }
    }
